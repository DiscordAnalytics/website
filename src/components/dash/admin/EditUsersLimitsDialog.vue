...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    toast.success('Limits successfully updated for the selected users.')
  } catch (e: any) {
...
<template>
  <Dialog :open="props.open" @update:open="$emit('update:open', $event)">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>Editing {{ $props.users.length }} users...</DialogTitle>
        <DialogDescription>You can update here the limits of the selected users.</DialogDescription>
      </DialogHeader>
        ...
        <FieldGroup>
          <VeeField v-slot="{ field, errors }" name="limit">
            <Field :data-invalid="!!errors.length">
              <FieldLabel for="limitInput">Bots limit</FieldLabel>
              <Input
                id="limitInput"
                v-bind="field"
